...
const Checkout = () => {
  ...
  // Fetch products and cart data on page load, we are familier with this below flow
  useEffect(() => {
      ...
      <Grid container>
        <Grid item xs md>
          <Box className="shipping-container" minHeight="100vh">
            ...
            {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Dislay either "Add new address" button or the <AddNewAddressView> component to add a new address in the currently added address */}

            {/* either use && operator OR if condition eg: if(newAddress.isAddingNewAddress=== false) then show button to add new address and 
            if(newAddress.isAddingNewAddress=== true) then show <AddNewAddressView> component. */}

            {/* using &&: if newAddress.isAddingNewAddress= false, then !false= true, and {true && true}=true, then this button jsx will be rendered 
            but in case of newAddress.isAddingNewAddress=true, no buttonn will be rendered*/}
